...
class Pokemath extends Program{
    ...
    int questionDegatAvecStab(Pokemon joueur, Pokemon adverse) {
        if(joueur.type == joueur.attaque.element) {
            println("Le pokemon " + joueur.name + " possede le type " + joueur.type + " et l'attaque " + joueur.attaque.name + " est de type " + joueur.attaque.element + " donc il y a STAB (donc il a un bonus de 50% sur son attaque)");
            println("Combien de degats fait l'attaque " + joueur.attaque.name + " avec STAB ?");
            return joueur.attaque.power * stab(joueur);
        }else{
            println("Le pokemon " + joueur.name + " possede le type " + joueur.type + " et l'attaque " + joueur.attaque.name + " est de type " + joueur.attaque.element + " donc il n'y a pas STAB");
    ...
    int stab (Pokemon joueur){
        int stab = 1;
        if(joueur.type == joueur.attaque.element){
            stab =joueur.attaque.power /2 ;
        }
        return stab;
    }
    ////////////////////////////////////////////////////////////////////////////////
    ...
    int demandeJoueur(){
        ...
        for(int idx=0; idx<nbLigneJoueur; idx++){
            if(equals(getCell(listeJoueur, idx, 0),nomJoueur) && nomJoueur != ""){
                
                idxJoueur = idx;
                joueurInscrit = true;
            }else if(equals(nomJoueur,"")){
                println("Vous n'avez pas saisi de nom, vous serez donc considéré comme un invité, vos scores ne seront pas sauvegardés.");
                nomJoueur = "Invite";
                println("Bienvenue " + nomJoueur + " ! :)");
                idxJoueur =idx;
                joueurInscrit = true;
            }
        }if(!joueurInscrit || equals(nomJoueur,"Invite")){
